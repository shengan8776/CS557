...
main( )
{
	...
	//end of gerstner wave code


	/*
	//PLEATS CODE - COMMENT THIS OUT WHEN USING GERSTNER WAVES
	// apply sin function to create pleates  //project3
    vec4 vert = gl_Vertex;
    vert.z = uA * (G - vert.y) * sin(2. * PI * (vert.x / uP));

    // calculate values to get new normals
    float dzdx = uA * (G - vert.y) * ((2. * PI) / uP) * cos(2. * PI * (vert.x / uP));
